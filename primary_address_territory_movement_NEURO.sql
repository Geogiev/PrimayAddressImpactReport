...
/****** Object:  View [dbo].[primary_address_territory_movement_NEURO]    Script Date: 12/16/2019 12:44:20 PM ******/
SET ANSI_NULLS ON
GO
...
--select * from  [dbo].[primary_address_territory_movement_NEURO]
 CREATE view [dbo].[primary_address_territory_movement_NEURO] as select
	a1.*,
	b1.*,
	v2.*,
	c.*,
...
from
	...
	from
		...
		where
			...
			and a.field in ('Primary_Address_Neuro__c')
			and a.oldvalue = 'true'
...
left outer join (
		...
		case
			...
			when SUBSTRING(territory_vod__c, len(territory_vod__c) -1, 2) = 'A1' then 'Renaissance'
			when SUBSTRING(territory_vod__c, len(territory_vod__c) -1, 2) = 'S1' then 'Resurgence'
			when SUBSTRING(territory_vod__c, len(territory_vod__c) -1, 2) = 'A2' then 'Revival'
		...
		case
			...
			when SUBSTRING(territory_vod__c, len(territory_vod__c) -1, 2) = 'T1' then 'DERM-ORTHO'
